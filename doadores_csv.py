...
# Se não achar o cpfs.csv, encerra o programa
if not os.path.exists('cpfs.csv'):
    print('Não foi localizado o arquivo cpfs.csv no diretório corrente. \n Encerrando o programa.')

else:
    df_cpf = pd.DataFrame()
    df_cpf = pd.read_csv('cpfs.csv', dtype = str, header = None)
    list_cpf = list(df_cpf[0])
    for num, cpf in enumerate(list_cpf):
    ...
    if len(lista) > 0:
        ...
        resultado = pd.concat([resultado, lista[0]])
    ...
    if len(lista2)>0:
        ...
        for l in lista2:
            ...
            resultado2 = pd.concat([resultado2, data_json])
